...
import { isManagedError } from '../error/error';

const handleError = (logLevel: LogLevel, e: any, contextName: string): void => {
    if (!isManagedError(e) && isNotLogged(e)) {
...
/*
Outputs only unintentional or malformed exceptions
 */
export function LogErrors(logLevel: LogLevel = 'error') {
