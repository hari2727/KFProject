...
function rebuild_library() {
    ...
    if [ -f ${REBUILD_MARKER_FULLPATH} -a "${FORCE}" != "true" ]; then
    ...
    if [ "${USE_LOCAL_LIB}" = "true" ]; then
        ...
        npm run rebuild:lib
        cd ${LIB_FULL_PATH} > /dev/null 2>&1
    fi

    if [ "${USE_LOCAL_THCLIB}" = "true" ]; then
        output "The 'use-local-thcllib' parameter was included, so rebuilding the kfhub_thcl_lib Project"
        output "The @kf-products-core/kfhub_thcl_lib library will be over-written with the current local kfhub_thcl_lib project contents..."
    ...
    if [ -f ${PKG_FULL_PATH}/package.json ]; then
...
function main() {
    ...
    if [ "${CL_ARGS}" != "" ]; then
        ...
        for ARG in "$@"; do
            case ${ARG} in
                use-local-lib)
                    USE_LOCAL_LIB="true"
                    ;;
                use-local-thclib|use-local-thcllib)
        ...
        if [ "${BAD_ARGS}" != "" ]; then
            output_error "Cannot perform tatmlib build for these arguments ${BAD_ARGS}. Aborting..."
        fi
    fi

    export USE_LOCAL_LIB USE_LOCAL_THCLIB
