...
import { FilterOption, SelectItemFlatten } from './sp-search.model';
...
export function mapSpSearchFilters(
    ...
    excludeFilters: SpSearchFilterEnum[],
    ...
    const defaultFilters = [
        ...
        [SpSearchFilterEnum.Grades],
        [SpSearchFilterEnum.Levels],
        [SpSearchFilterEnum.Functions, SpSearchFilterEnum.SubFunctions],
        [SpSearchFilterEnum.SearchSource],
        [SpSearchFilterEnum.Industry],
        [SpSearchFilterEnum.Language],
    ];
    ...
    return _.chain(defaultFilters).filter(f => !excludeFilters.some(ef => ef === f[0])).map(type => {
        ...
        return {
            type,
            term: type[0] === SpSearchFilterEnum.Industry ? _.toLower(`pm.${type.join('_')}`) : `lib.${type.join('_')}`,
            items: mapOptionsToSelectItems(
                filter.options as unknown as FilterOption[],
                [type.join('_')],
                checkedOptions,
                ['searchOn', 'subOptions'],
                type[0] === SpSearchFilterEnum.ProfileType
...
export function getSelectedFiltersFlatten(filters: FiltersState) {
    const mapSelectItemFn = (type: SpSearchFilterEnum, term: string, item: SelectItem<string>, parentId?: string | number) =>
    ...
    return _.flatMapDeep(
        ...
        f => f.items.filter(item => item.checked).map(item =>
            _.isArray(item.value)
                ...
                : mapSelectItemFn(
                    f.type[0],
                    f.type[0] === SpSearchFilterEnum.Industry ? _.toLower(`pm.${f.type[0]}`) : `lib.${f.type[0]}`,
