/* eslint-disable */
import { Observable, ConnectableObservable, Subject, forkJoin, of, Subscription, Observer, lastValueFrom } from 'rxjs';
...
import { KfTarcSuccessProfileDetailService, KfTarcSPViewOptions } from './kftarc-success-profile-detail.service';
...
import { KfCategoryNode, KfSubCategoryNode, MAX_COMPETENCIES, SkillsBarChartWidth, TREE_MAP_COLOR_PALETTE, KfEditSuccessProfileOptionsEnum, KfSuccessProfileStatus, KFUcpCollabStatusEnum, KfBCRecords, KfBCPage } from './kftarc-sp-detail.interface';
...
import { findSelectedLocale } from './kftarc-sp-detail.pure';
...
import { ProductTypeEnum, UAMSelectPermissionCodes } from '../../../../../../pages/sp-search/sp-search.constant';
import { SelectAssessProducts, SelectAssessProductIdsEnum, SelectAssessProductTypeEnum, SelectAssessProductIdEnum } from '../../../../../../pages/sp-search/sp-search.model';
...
@Component({
    selector: 'kftarc-spdetail',
    templateUrl: './kftarc-sp-detail.component.html',
    styleUrls: ['./kftarc-sp-detail.component.scss'],
    providers: [DecimalPipe],
})
export class KfTarcSPDetailComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    behavioralCompsRecords: KfBCPage[] = [];
    skillsRecords: KfBCPage[] = [];
    ...
    public jobTitleMaxLengthWarningText: string;
    ...
    public isDeselectAllOptionSelected = false;
    ...
    public showMaxJobsTooltip = false;
    ...
    constructor(
        ...
        this.jobTitleMaxLengthWarningText = this.translationService.get('lib.jobTitleLengthWarningText', {
            JOB_TITLE_MAX_LENGTH,
        });

        const navigation = this.router.getCurrentNavigation();
        if (navigation && navigation.extras && navigation.extras.state && Object.keys(navigation.extras.state).length !== 0) {
    ...
    async ngOnInit() {
        ...
        this.activatedRoute.data
            .subscribe((spData: Data) => {
                const navigation = this.router.getCurrentNavigation();
                if(navigation && this.makeAuditCall) {
                    this.spSharedService.callViewAuditLog(this.applicationTypeEnum.PM, this.applicationTypeEnum.SM, [spData['spData']?.id], true);
                }
        ...
        this.getJobId.push(this.successProfile.id);
        if (this.makeAuditCall) {
            this.spSharedService.callViewAuditLog(this.applicationTypeEnum.PM, this.applicationTypeEnum.SM, this.getJobId, true);
        }
    ...
    async initializeSuccessProfile(data: KfISuccessProfile) {
        ...
        const { Client: { ClientId }, User: { UserId } } = this.authService.getSessionInfo();
        const [selectResponse, assessResponse] = await lastValueFrom(
            forkJoin([
                this.successProfileService.getUamSelectAssessProducts(ClientId,
                                                                      UserId, SelectAssessProductIdEnum.ProductId, SelectAssessProductIdsEnum.Select),
                this.successProfileService.getUamSelectAssessProducts(ClientId,
                                                                      UserId, SelectAssessProductIdEnum.ProductId, SelectAssessProductIdsEnum.Assess),
            ])
        );

        this.selectProducts = selectResponse.user?.productLine?.flatMap(sel => sel?.countries[0]?.product) || [];
        this.assessProducts = assessResponse.user?.productLine?.flatMap(ases => ases?.countries[0]?.product) || [];
        ...
        const skillSection = this.successProfile.sections
            .find(section => section.code === 'TECHNICAL_SKILLS');
        if(skillSection?.subCategories){
        ...
        this.showMaxJobsTooltip = false;

        if(params?.maxJobs) {
            this.showMaxJobsTooltip = true;
        }
        ...
        this.successProfileService.getProfileVersion(this.successProfile.id, 'background')
            ...
            .subscribe((response: KfTarcProfileVersion[]) => {
                if (response) {
                    ...
                    if (this.successProfile.parentJobDetails && response.length > 0 && parseFloat(this.successProfile?.parentJobDetails?.version) < parseFloat(response[response.length - 1]?.version)) {
            ...
            .subscribe(([res, hasJobPricingAccess]: [any | null, boolean]) => {
                if (res?.productTypes) {
                    ...
                    if (!this.collabData || this.collabData?.data && !this.collabData.data.collabIdAvailable) {
                        const isProductsEmpty = (products: any[]): boolean => products.every(product => product === null || product === undefined);

                        const result = productTypes.map(productType => ({
                            id: productType.id,
                            name: productType.name,
                            isProductsEmpty: isProductsEmpty(productType.products)
                        }));
                        const checkForallProductsEmptyTrue = result.every(item => item.isProductsEmpty === true);
                        if(checkForallProductsEmptyTrue) {
                            this.canCreateAssessmentProject = false;
                        } else {
    ...
    navigateToSuccessProfile(id): void {
        ...
        if (this.collabEdit) {
            ...
            this.router.navigate([`tarc/sp/detail/${id}`], {state: { restrictAuditCall: 'yes' }});
        }
    }
    ...
    openCopyModal(): void {
        ...
        // Fetch profile collections before opening modal
        this.successProfileService.getClientProfileCollections().subscribe(
            collections => {
    ...
    public closeCopyModal(): void {
        this.isCopyTextAdded = false;
        this.copySPTitle = this.successProfile.title;
    }

    copySuccessProfile(): void {
        let copyTitle = this.copySPTitle;
        this.successProfile.titleChanged = true;
        if (this.collabEdit) {
            copyTitle = `${this.copySPTitle} ${this.translate.instant('lib.sps')}`;
            if (copyTitle.length > JOB_TITLE_MAX_LENGTH) {
                const profileTitleWarningText =  this.translationService.get('lib.jobTitleLengthWarningText', {
                    JOB_TITLE_MAX_LENGTH,
                });
                this.growlService.createInteractiveErrorMessage(
                    profileTitleWarningText, this.translationService.get('pm.editFailedMessage'), KfGrowlMessageType.ERROR, 'data', 10000);
                this.isCopyModalOpen = false;
                this.successProfile.titleChanged = false;
                return;
            }
            this.copySPTitle = copyTitle;
            this.jobCode = '';
        ...
        this.loadingService.blockOnPendingRequests(
            async () => {
                try {
                    this.copySP = _.cloneDeep(this.successProfile);
                    this.copySP.title = copyTitle;
                    this.copySP.jobCode = this.jobCode;
                    if (this.copySP.sections) {
                        this.copySP.sections.forEach((section) => {
                            if (section.code === 'TECHNICAL_SKILLS' && section.subCategories) {
                                section.subCategories.forEach((subCategory) => {
                                    if (subCategory?.dependents) {
                                        subCategory?.dependents.forEach((dependent) => {
                                            delete dependent?.clientJobSkillComponentId;
                                        });
                                    }
                                });
                            }
                    ...
                    const { id } = await this.saveEdit(true, this.copySP, localeFromFilter, originalLocale);
                    this.spSharedService.callViewAuditLog(this.applicationTypeEnum.PM, this.applicationTypeEnum.SM, [id], true);
                    this.isCopyModalOpen = false;
                    this.successProfile.title = copyTitle;
    ...
    async saveEdit(isClone: boolean, modifiedSP: KfISuccessProfile, localeFromFilter?: string, originalLocale?: string) {
    ...
    downloadInterviewGuide(): void {
        const behavioralSection = this.successProfile.sections
            .find(section => section.code === 'BEHAVIORAL_SKILLS');
        const skillsSection = this.successProfile.sections
            .find(section => section.code === 'TECHNICAL_SKILLS');
        this.behavioralCompetenciesSection = behavioralSection;
        //  this.behavioralCompsRecords = this.behariouralComListRecords(this.behavioralCompetenciesSection, 1)
    ...
    behariouralComListRecords(type:string, behaviouralSection: KfISpSection, page: number):KfBCPage[] {
        const allRecords: KfBCRecords[] = [];
        if(behaviouralSection.hasOwnProperty('subCategories')) {
            const grouped = behaviouralSection.subCategories.forEach(subCat => {
    ...
    toggleEditSuccessProfile(toggleEvent: { event: boolean; editSuccessProfileFormValue: string}): void {
        ...
        this.loadingService.blockOnPendingRequests(() => {
            if (toggleEvent.event && toggleEvent.editSuccessProfileFormValue === KfEditSuccessProfileOptionsEnum.STANDARD) {
                this.successProfileService.backupSuccessProfile(this.successProfile);
                if (typeof this.successProfileService.backupLastSuccessProfile === 'function') {
                    this.successProfileService.backupLastSuccessProfile(this.successProfile, 'BACKUP');
                }
                this.cacheAssessmentSettings();
                this.navigateToEditPage();
            } else if (toggleEvent.event && toggleEvent.editSuccessProfileFormValue === KfEditSuccessProfileOptionsEnum.COLLABORATIVE) {
    ...
    showCreateJobModel(): void {
        ...
        if (this.currentURL.includes('add-job')) {
            ...
            KfNavigationUtil.navigateMenu(this.router, '/arch', path, {
                queryParams: {
                    spId: this.successProfile.id,
                    expanded: this.activatedRoute.snapshot.queryParams?.expanded ?? false
                },
            });
        } else if (this.currentURL.includes('ladder')) {
            path = '/job/ladder/compare-select';
            const selectedJobs = this.activatedRoute.snapshot.queryParams?.selectedJobs;
            const queryParams = selectedJobs?.length > 0 ? { selectedJobs, spId: this.successProfile.id } : { spId: this.successProfile.id };
            KfNavigationUtil.navigateMenu(this.router, '/arch', path, { queryParams });
        }
    }
    ...
    back(): void {
        ...
        if (this.isSliderOpen) {
            ...
            if (this.currentURL.includes('add-job')) {
                path = '/job/create';
                const queryParams = this.activatedRoute.snapshot.queryParams?.checked === 'true' ? { spId: this.successProfile.id, retainState: true, backButtonClicked: true } :
                    { spId: params?.selectedProfileIdIsChecked, checked: false };
                KfNavigationUtil.navigateMenu(this.router, '/arch', path, { queryParams });
            } else if (this.currentURL.includes('ladder')) {
                path = '/job/ladder/compare-select';
                const selectedJobs = this.activatedRoute.snapshot.queryParams?.selectedJobs;
                selectedJobs?.length > 0 ? KfNavigationUtil.navigateMenu(this.router, '/arch', path, { queryParams: { selectedJobs } })
                    : KfNavigationUtil.navigateMenu(this.router, '/arch', path);
            }
        }
    }
    ...
    async getMarketInsights(): Promise<KfIMarketInsightsResults> {
        return this.marketInsightsDataService.getData(
            {
                successProfileId: this.successProfile?.id,
                countryId: parseInt(this.selectedCountry?.id, 10),
                requestType: 'hidden',
                forced: true,
            },
        );
    ...
    public deselectAllOptionSelected(event: boolean): void {
        this.isDeselectAllOptionSelected = event;
    }
}
